...
def components(candidate):
    if set(candidate) != {"instructions", "demonstrations"}:
        raise ValueError("GEPA may change only instructions and demonstrations")
    if not isinstance(candidate["instructions"], str) or not candidate["instructions"].strip():
        raise ValueError("Instructions must be nonempty text")
    examples = json.loads(candidate["demonstrations"])
    if not isinstance(examples, list):
        raise ValueError("Demonstrations must be an array")
    for example in examples:
        if set(example) != {"input", "action"}:
            raise ValueError("Invalid demonstration fields")
        if set(example["input"]) != {"inheritedInstructions", "brief", "context", "tools"}:
            raise ValueError("Invalid demonstration inputs")
        if not all(isinstance(value, str) for value in example["input"].values()):
            raise ValueError("Demonstration inputs must be text")
        Action.model_validate(example["action"])
    return {"instructions": candidate["instructions"], "demonstrations": examples}
...
class CampaignAdapter:
    ...
    def make_reflective_dataset(self, candidate, eval_batch, components_to_update):
        return {
            component: [
                {"Inputs": result["caseId"], "Outputs": result, "Feedback": result["feedback"]}
...
def learn(request, exchange):
    ...
    seed = {
        "instructions": request["candidate"]["instructions"],
        "demonstrations": json.dumps(request["candidate"]["demonstrations"]),
    }
    result = optimize(
        seed_candidate=seed,
        trainset=[case for case in cases if case["role"] == "training"],
        valset=[case for case in cases if case["role"] == "validation"],
        adapter=CampaignAdapter(exchange),
