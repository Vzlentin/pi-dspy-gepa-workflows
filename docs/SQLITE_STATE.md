...
│       ├── sessions/<stage>-<n>/ # one fresh Pi transcript per stage (plus prompt files under Herdr)
│       ├── dspy-traces.jsonl    # every recorded stage: inputs, typed output, LM history
│       ├── python.log
...
- `candidates`: content-addressed immutable stage instructions, validated review demonstrations, fixed-program identity, and runtime provenance. Exactly four stages are present; only review carries demonstrations.
...
Each stage's Pi JSONL transcript under `runs/<id>/sessions/` is the conversation authority for that stage; the database stores no replayable copy of operations. Complete DSPy traces, Python diagnostics, verification logs, review evidence and evaluation source artifacts live in private files. Each evidence object records its content fingerprint and output locations. `campaign status` computes freshness from current source.
...
There is no per-operation replay or automatic daemon recovery. Explicit resume reuses the working tree, restarts the recorded stage in a fresh session, and starts a fresh Python process. A claimed live owner prevents concurrent writers. The SQLite store imports no Pi, runtime, or learning modules.
