...
- Respect dependency boundaries in `slophammer.yml`: state and campaign are Pi-independent;
  runtime never imports learning; the launcher composes runtime and learning.
...
pi-dspy-gepa-workflows is in alpha. Until the repository explicitly leaves alpha:
